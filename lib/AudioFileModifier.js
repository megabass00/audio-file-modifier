...
module.exports = class AudioFileNormalize {
  constructor(options) {
    ...
    this.printLogo();
    this.log('AudioFileModifier'.yellow + 'Initialized with config'.gray, this.config);
  ...
  async splitSilences(pathToFile) {
    ...
    return new Promise((resolve, reject) => {
      ...
      ffmpeg(inFile)
        .audioFilters('silenceremove=start_periods=1:start_duration=1:start_threshold=-60dB:detection=peak')
        .audioFilters('aformat=dblp')
  ...
  async getSampleRate(pathToFile) {
    ...
    return metadata.sampleRate.toPrecision();
  }

  async getBitRate(pathToFile) {
    const metadata = await this.getMetadata(pathToFile);
    return metadata.bitrate ? metadata.bitrate.toPrecision() / 1024 : 0;
  }

  async getDuration(pathToFile) {
    const metadata = await this.getMetadata(pathToFile);
    return metadata.duration ? metadata.duration.toPrecision() : 0;
  }

  async getNumberOfChannels(pathToFile) {
    const metadata = await this.getMetadata(pathToFile);
    return metadata.numberOfChannels ? metadata.numberOfChannels.toPrecision() : 1;
  }
  ...
  async getMetadata(pathToFile) {
    return new Promise((resolve, reject) => {
      try {
        ...
        this.error(`Error getting matadata on ${this._urlParts(pathToFile).base.black}`, err);
        resolve({
          bitrate: 0,
          numberOfChannels: 0,
          sampleRate: 0,
        ...
        this.error('It has been impossible get metadata on', this._urlParts(pathToFile).base.black);
        resolve({
          bitrate: 0,
          numberOfChannels: 0,
          sampleRate: 0,
